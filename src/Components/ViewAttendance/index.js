...
const ViewAttendance = () => {
    ...
    <ViewChildrenContainer>
      {times.map((item) => (
        <ChildrenStatus
          key={item}
          style={{
            background: "rgb(247,236,232)",
            border: "none",
            backgroundColor: "rgba(219, 219, 219, 0.27000001072883606)",
          }}
        >
          <Skeleton
            width="50px"
            height="50px"
            borderRadius="50%"
            style={{ marginTop: 10, alignSelf: "flex-start", marginRight: 20 }}
          />
          ...
          <Skeleton
            ...
            height="30px"
            borderRadius="8px"
            style={{ marginLeft: "auto", marginBottom: 10, marginTop: 10 }}
          />
    ...
    <FailureContainer>
      ...
      <ErrorMessage>
        {language === "english"
          ...
          : "మీరు వెతుకుతున్న సమాచారాన్ని మేము కనుగొనలేకపోయాము."}
      </ErrorMessage>
  ...
  const renderDateFailureView = (language) => (
    <FailureContainer>
      ...
      <TryAgainBtn type="button" onClick={refershDates}>
        {language === "english" ? "Try again" : "మళ్ళీ ప్రయత్నించండి"}
      </TryAgainBtn>
    </FailureContainer>
  );
  ...
  const renderSuccessChildrensStatusView = (language) => {
      ...
      switch (presents) {
        case apiResponseData.lead:
          return language === "english"
            ? `congrats ${name.split(" ")[0]}, you are in lead.`
            : `అభినందనలు ${
                getTeluguWords(name).split(" ")[0]
              }, మీరు ముందంజలో ఉన్నారు`;
        case apiResponseData.min:
          return language === "english"
            ? `${name.split(" ")[0]}, your attendance is very low.`
            : `${
                getTeluguWords(name).split(" ")[0]
              }, మీ హాజరు చాలా తక్కువగా ఉంది`;
        case occur:
          return language === "english"
            ? `congrats ${
                name.split(" ")[0]
              }, But you're one step away from peak attendance.`
            : `అభినందనలు ${
                getTeluguWords(name).split(" ")[0]
              }, కాని మీరు అత్యధిక హాజరుకు ఒక అడుగు వెనకబడి ఉన్నారు`;
        default:
          return "";
      }
    };
    ...
    return (
      <>
        <ViewChildrenContainer>
          {apiResponseData.childrenStatusArray
            .slice(startIndex, endIndex)
            .map((item) => (
              <>
                {apiResponseData.lead === item.presents && (
                  <ReactTooltip
                    id={item.name}
                    place="top"
                    className={`tool ${
                      apiResponseData.lead === item.presents && "border"
                    }`}
                  />
                )}
                {apiResponseData.min === item.presents && (
                  <ReactTooltip
                    id={item.name}
                    place="top"
                    className="tool-red"
                ...
                {apiResponseData.lead - 1 === item.presents && (
                  <ReactTooltip
                    id={item.name}
                    place="top"
                    className="tool-occur"
                  />
                )}
                <ChildrenStatus
                  key={item.name}
                  occur={(
                    apiResponseData.lead - 1 ===
                    item.presents
                  ).toString()}
                  low={(apiResponseData.min === item.presents).toString()}
                  data-tooltip-id={item.name}
                  data-tooltip-content={correspondingMessage(
                    item.presents,
                    item.name
                  )}
                  high={(apiResponseData.lead === item.presents).toString()}
                  count={item.presents}
                >
                  {apiResponseData.lead === item.presents && (
                    <RiMedalFill className="star" />
                  )}
                  {/* <FirstLetterContainer
                    occur={(
                      apiResponseData.lead - 1 ===
                  ...
                  >  {apiResponseData.childrenStatusArray.indexOf(item) + 1}
                  </FirstLetterContainer>
                  */}
                  <ImageChildren
                    alt="childrenImage"
                    ...
                    src={
                      item.gender === "MALE"
                        ? "https://res.cloudinary.com/dkrpgt9kd/image/upload/v1706681450/a1rmhceyfccwmj04gn9g.png"
                  ...
                  <ChildrenName
                    ...
                    high={(apiResponseData.lead === item.presents).toString()}
                    style={{ fontSize: language === "తెలుగు" && `20px` }}
                  >
                    {language === "english"
                      ? item.name
                  ...
                  <Presents>
                    {language === "english" ? "Present" : "హాజరు"}:{" "}
                    <AttendanceCount count={item.presents}>
                      {item.presents}
                    </AttendanceCount>
                  </Presents>
                </ChildrenStatus>
        ...
        <PagenationContainer>
          ...
          >
            <TbPlayerTrackPrev color="#C5AE5E" />
          </PrevButton>
          <ReactTooltip
            id="next"
            place="right"
          ...
          <NextButton
            ...
            disabled={endIndex >= apiResponseData.childrenStatusArray.length}
          >
            <TbPlayerTrackNext color="#C5AE5E" />
          </NextButton>
        </PagenationContainer>
      </>
    );
  ...
  const renderDateViceChildernStatusView = (language) => {
    ...
    return (
      <>
        <ViewChildrenContainer>
          {apiResponseData.dateViceDetailsArray
            .slice(start, end)
            .map((item) => (
              <DailyStatusChildern key={item.name} present={item.presents}>
                <FirstLetterContainer
                  ...
                  style={{ fontWeight: 600 }}
                >
                  {apiResponseData.dateViceDetailsArray.indexOf(item) + 1}
                </FirstLetterContainer>
                <ChildrenName
                  style={{ fontSize: language === "తెలుగు" && `20px` }}
                >
        ...
        <PagenationContainer>
          ...
          <NextButton
            ...
            data-tooltip-content={"Next"}
            onClick={handleNext}
            disabled={end >= apiResponseData.dateViceDetailsArray.length}
          >
            <TbPlayerTrackNext color="#fff" />
          </NextButton>
        </PagenationContainer>
      </>
    );
  ...
  const onSearchDate = () => {
    if (searchDate === "") {
      ...
      handleSearch(true);
      const filteredDatesArray = apiResponseData.datesArray.map(
        (item) => item.date !== undefined && item.date
      );
      ...
      {(value) => {
        ...
        return (
          <ViewContainer>
            ...
            <Heading>
              <HighlatedText>
                {language === "english" ? "Children" : "పిల్లల"}
              </HighlatedText>{" "}
              {language === "english" ? "Attendance details" : "హాజరు వివరాలు"}
            </Heading>
            ...
            {apiResponseData.dateError ? (
              renderDateFailureView()
            ) : (
              <>
                <SelectDateContainer>
                  <SelctDateHead>
                    {language === "english"
                      ? "Attendance details by Dates"
                      : "తేదీల వారీగా హాజరు వివరాలు"}
                  </SelctDateHead>
                  <SearchDataContainer>
                    <SearchDateInput
                      type="date"
                ...
                {isSearch ? (
                  searchErr ? (
                    <DateContainer>
                      ...
                      <SelectDateText>
                        {language === "english"
                          ? "Your search date cannot in backend"
                          : "మీ శోధన తేదీ ఉనికిలో లేదు"}
                      </SelectDateText>
                    </DateContainer>
                  ) : (
                    <Skeleton.SkeletonThemeProvider
                  ...
                  <DateContainer>
                    <DateImage
                      alt="select date"
                      src="https://res.cloudinary.com/dkrpgt9kd/image/upload/v1709103464/bxm9dnwyw5h0qmkk2llb.png"
                    />
                    <SelectDateText>
                      {language === "english"
                        ? "Search by date and get your details"
                        : "తేదీ ద్వారా శోధించండి మరియు వివరాలను పొందండి"}
                    </SelectDateText>
                  </DateContainer>
                )}
              </>
